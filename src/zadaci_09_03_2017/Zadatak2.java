...
public class Zadatak2 {
	...
	public static void main(String[] args) {
		System.out.print("Upisite integer m: ");
		Scanner input = new Scanner(System.in);
		try {
			int m = input.nextInt();
			if (m <= 1) {
				System.out.println("Unesite broj veci od 1 !");
			} else {
				ArrayList<Integer> factors = new ArrayList<>();
				getFactors(m, factors);
				int smallestSquare = smallestSquare(factors);
				System.out.println("Najmanji broj n za m * n da bude perfect square je: " + smallestSquare);
